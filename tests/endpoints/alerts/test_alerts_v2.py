...
class TestAlertsV2Endpoint:
    """
    Tests for the /2.0/alerts/ endpoint
    """
    ...
    def test_get_200(self, httpx_mock: HTTPXMock, get_response_obj):
        """
        Hits the /2.0/alerts/{alert_id} endpoint
        using the correct attributes
        """
        # setup mock response
        expected_response = get_response_obj(
            "tests/endpoints/alerts/fixtures/get_v2_200.json",
        )
        httpx_mock.add_response(status_code=200, json=expected_response)

        # make the call
        status_code, alert = self.zfapi.alerts_v2.get(
            alert_id=39229922,
        )
        ...
        assert mock_request.url == ("https://api-test.zerofox.com/2.0/alerts/39229922")
    ...
    def test_simple_list_200(self, httpx_mock: HTTPXMock, get_response_obj):
        """
        Hits the /2.0/alerts/{query_params} endpoint
        using the correct attributes
        """
        # setup mock response
        expected_response = get_response_obj(
            "tests/endpoints/alerts/fixtures/list_v2_200.json",
        )
        httpx_mock.add_response(status_code=200, json=expected_response)

        # make the call
        status_code, alerts, pagination = self.zfapi.alerts_v2.simple_list(
            query_params={
                "enterprise_id": 2831,
                "page_size": 2,
            },
        ...
        assert mock_request.url == (
            "https://api-test.zerofox.com/2.0/alerts/?enterprise_id=2831&page_size=2"
        )
        assert mock_request.headers == {
            "host": "api-test.zerofox.com",
            "accept": "*/*",
